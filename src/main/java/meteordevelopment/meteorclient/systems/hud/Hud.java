...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    public static final HudGroup GROUP = new HudGroup("MinClient");
    ...
    private final Setting<Boolean> hideInMenus = sgGeneral.add(new BoolSetting.Builder()
        .name("hide-in-menus")
        .description("Hides the MinClient hud when in inventory screens or game menus.")
        .defaultValue(false)
        .build()
    );
    ...
    private void resetToDefaultElementsImpl() {
        ...
        // Top Left
        add(WatermarkHud.INFO, 4, 4, XAnchor.Left, YAnchor.Top);
        add(SessionStatsHud.INFO, 4, 4 + h * 3, XAnchor.Left, YAnchor.Top);
        add(MeteorTextHud.FPS, 4, 4 + h * 8, XAnchor.Left, YAnchor.Top);
        add(MeteorTextHud.TPS, 4, 4 + h * 9, XAnchor.Left, YAnchor.Top);
        add(MeteorTextHud.PING, 4, 4 + h * 10, XAnchor.Left, YAnchor.Top);
        add(MeteorTextHud.SPEED, 4, 4 + h * 11, XAnchor.Left, YAnchor.Top);
